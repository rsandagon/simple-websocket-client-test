...
var clientId = "9c42512f406d4741a31b48a89df21sd3"; //can be windows name
...
function refreshList(){
    ...
    if(isBusy){
        queueCollection.forEach((item,index)=>{
            dom += `<li class="flex p-6 border justify-between border-blue-200">
            <div class="ml-4 flex gap-4 flex-1">
                <h1 class="grow text-xl">Queue No. ${index + 1}</h1>
                <div class="w-4/5 bg-gray-300 rounded-full h-2.5">
        ...
        queueCollection.forEach((item,index)=>{
            ...
            <div class="ml-4 flex gap-4 flex-1">
                ...
                <button onclick="removeItem(${index})" type="button" class="font-medium text-indigo-600 hover:text-indigo-500">Remove</button>
            </div>
            </li>`;
        });
    }    
